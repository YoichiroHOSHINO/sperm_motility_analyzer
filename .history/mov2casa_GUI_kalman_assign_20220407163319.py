...
def makeTracks(df, pnt, h):
    ...
    for f in range(start_frame + 1, end_frame):
        for s in [0,1]:
            ...
            #print (len(nx), len(pre_px))
            px, pvx = Kalman(np.array(nx, dtype=float32), np.array(pre_px, dtype=float32), np.array(pre_pvx, dtype=float32))
            py, pvy = Kalman(np.array(ny, dtype=float32), np.array(pre_py, dtype=float32), np.array(pre_pvy, dtype=float32))
