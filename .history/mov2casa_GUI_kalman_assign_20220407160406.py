...
def makeTracks(df, pnt, h):
        ...
        for s in [0,1]:
            ...
            for t in range(maxjumpframes):
                if f - start_frame - t >= 1:
                    ...
                    layer_stack.extend([t]*len(p_idx[0]))
                    #n_idx_stack.extend(list(n_idx[0]))
            ...
            p_angle_x_list, p_angle_y_list = df[ps, h.index('Angle_x_list')], df[ps, h.index('Angle_y_list')]
            n_angle_x_list, n_angle_y_list, angle = makeAngle(p_angle_x_list, p_angle_y_list, Ave_x, Ave_y, f)
            df[ns, h.index('Angle_x_list')] = n_angle_x_list.astype('U32')
            df[ns, h.index('Angle_y_list')] = n_angle_y_list.astype('U32')
            df[ns, h.index('angle')] = angle.astype('U32')

            df_wD = df[df[:,h.index('Framelength')].astype(np.int64) >= max_frame-1]
            P_wD = np.unique(df_wD[:,h.index('Point')].astype(np.int64))
            No_P = P_wD.shape[0]
                
            pre_Ave_x = df[ps, h.index('Ave_x')].astype(np.float32)
            pre_Ave_y = df[ps, h.index('Ave_y')].astype(np.float32)
            pre_Ave_runLength = df[ps, h.index('Ave_RunLength')].astype(np.float32)
            Ave_span = np.sqrt(pow(Ave_x.astype(np.float32) - pre_Ave_x,2)+pow(Ave_y.astype(np.float32) - pre_Ave_y,2))
            Ave_Length = Ave_span * microscale
            df[ns, h.index('Ave_Length')] = Ave_Length.astype('U32')
            Ave_RunLength = pre_Ave_runLength + Ave_Length
            df[ns, h.index('Ave_RunLength')] = Ave_RunLength.astype('U32')
    ...
    for s in [0,1]:
        ...
        dfunfixidx = np.where((df[:,h.index('Frame')] == 0)&(df[:,h.index('motile')] == motile))
        if len(dfunfixidx[0]) > 0:
            for p in tqdm(dfunfixidx[0]):
                ...
                while f < end_frame or notrack == 0:   # フレームⅠ以上の処理
                    ...
                    for t in range(mxjf):
                        ...
                        # 近接範囲にある粒子一覧を取得
                        nxtnrpsidx = np.where((df[:,h.index('Frame')] == df[p,h.index('Frame')] + t)&(df[:,h.index('fix_next')] != 1)&(df[:,h.index('motile')] == motile))
