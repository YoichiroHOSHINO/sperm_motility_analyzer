...
def nichika(img):
    ...
    thresh_dark = cv2.erode(thresh2, kernel, iterations = dark_erosion_iter)
    thresh_dark = cv2.dilate(thresh2, kernel, iterations = dark_dilate_iter)
...
def makeTracks(df, pnt, h):
    ...
    for ff in range(start_frame, end_frame):
        for s in [0,1]:
            ...
            #    for mv in p_idx[0]:
            #        if df[mv, h.index('Point')] == 0:
            #            df[mv, h.index('Point')] = pnt
            #            pnt += 1
            ...
            for t in range(maxjumpframes):
                if f - t >= 0:
                    ...
                    #print (len(n_idx[0]), len(n_idx_stack))
                    ...
                    if len(p_idx[0]) > 0 and len(n_idx[0]) > 0:
                        ...
                        pxs = df[p_idx[0], h.index('pred_x')]   # カルマン予測
                        pys = df[p_idx[0], h.index('pred_y')]
            ...
            if matrix_xyz.shape[0] > 0:
                ...
                if len(p_idx_stack) <= len(n_idx_stack):
                    pairs = len(p_assign_idxs)
                else:
                    pairs = len(n_idx_stack)

                #print (matrix_xyz.shape, len(p_idx_stack), len(n_idx_stack), len(layer_stack), len(p_assign_idxs), len(n_assign_idxs))
                ...
                for i in range(pairs):
                    ...
                    if cost < mobsearchrange:
                        ...
                        layers.append(layer_stack[n_assign_idxs[i]])

                for s in range(len(ps)):
                    pre_point = df[ps[s], h.index('Point')]
                    if pre_point == 0:
                        df[ps[s], h.index('Point')] = pnt
                        df[ns[s], h.index('Point')] = pnt
                        pnt += 1
                    else:
                        df[ns[s], h.index('Point')] = pre_point
                ...
                df[ns, h.index('connect_index')] = df[ps, h.index('index')]
                ...
                for i in range(len(ps)):
                    psi = ps[p]
                    nsi = ns[p]
                    pre_px, pre_py, pre_pvx, pre_pvy = df[psi, h.index('pred_x')], df[psi, h.index('pred_y')], df[psi, h.index('pred_vx')], df[psi, h.index('pred_vy')]
                    n_x, n_y = df[nsi, h.index('x')], df[nsi, h.index('y')]
                    #print (len(nx), len(pre_px), len(pre_pvx))
                    px, pvx = Kalman(n_x, pre_px, pre_pvx)
                    py, pvy = Kalman(n_y, pre_py, pre_pvy)
                    df[nsi, h.index('pred_x')] = px
                    df[nsi, h.index('pred_y')] = py
                    df[nsi, h.index('pred_vx')] = pvx
                    df[nsi, h.index('pred_vy')] = pvy
                #
